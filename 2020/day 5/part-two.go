...
func main() {
	...
	seatsChecked := make(map[int]struct{}, ROWS*COLS)
	...
	for seat := range seatsChecked {
		_, nextSeatTaken := seatsChecked[seat+1]
		if !nextSeatTaken {
			fmt.Printf("Your seat: %d\n", seat+1)
			break
		}

		_, previousSeatTaken := seatsChecked[seat-1]
		if !previousSeatTaken {
			fmt.Printf("Your seat: %d\n", seat-1)
			break
		}
	}
